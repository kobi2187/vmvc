...
suite "parsing utils":
  ...
  test "cmd + sub 1,2,3 + config":
    let ex4 = parseCommand("convert audio for sure -input:mp3 -output:wav -bitrate:224") # cmd sub1 sub2 sub3 configs
    assert ex4.error.ok
    
    check ex4.command == "convert"
    check ex4.subCommands == @[ "audio", "for", "sure" ]
    check ex4.arguments == { "input":"mp3" , "output":"wav", "bitrate":"224"}.newTable
